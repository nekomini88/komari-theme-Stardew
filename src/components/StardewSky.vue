...
import { isFog, isNight, isRain, isSnow, isThunder, season as seasonRef, setWeather } from '@/composables/useStardewAtmosphere'
...
const tint = computed(() => {
  ...
  if (s === 'spring')
    return `oklch(0.72 0.06 138 / 0.28)`
  if (s === 'summer')
    return `oklch(0.8 0.08 92 / 0.24)`
  if (s === 'autumn')
    return `oklch(0.78 0.08 72 / 0.24)`
...
.stardew-moon {
  ...
  animation: stardew-sky-arc 32s linear infinite;
}
.stardew-sun {
  animation: stardew-sun-rise 28s ease-in-out infinite alternate;
}
.stardew-moon {
  animation: stardew-moon-rise 28s ease-in-out infinite alternate;
}
...
.stardew-rain {
  ...
  background: repeating-linear-gradient(
    180deg,
    rgba(180, 210, 255, 0.45) 0px,
    rgba(180, 210, 255, 0.45) 2px,
    transparent 2px,
    transparent 9px
  );
...
.stardew-fog {
  ...
  inset: -20%;
  background:
    radial-gradient(circle at 30% 50%, rgba(255, 255, 255, 0.35) 0 30%, transparent 60%),
...
@keyframes stardew-sky-arc {
  0% {
    transform: translateX(-50%) translateX(-10vw);
  }
  50% {
    transform: translateX(-50%) translateX(10vw);
  }
...
@keyframes stardew-snowdrift {
  0% {
    transform: translate(0, 0);
    opacity: 0.8;
  }
  100% {
    transform: translate(12px, 12vh);
    opacity: 0.2;
  }
}
